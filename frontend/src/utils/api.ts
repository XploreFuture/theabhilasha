...
import type { Event, CreateEventPayload, UpdateEventPayload } from "../types/api";
...
export const getEvents = async (): Promise<Event[] | null> => {
  return fetchWithAuth<Event[]>("/events", { method: "GET" });
};

// Get event by ID
export const getEventById = async (id: string): Promise<Event | null> => {
  return fetchWithAuth<Event>(`/events/${id}`, { method: "GET" });
};

// Create new event
export const createEvent = async (payload: CreateEventPayload): Promise<Event | null> => {
  return fetchWithAuth<Event>("/events", {
    method: "POST",
    headers: { "Content-Type": "application/json" }, // ✅ always set headers
    body: JSON.stringify(payload),
  });
};

// Update event
export const updateEvent = async (id: string, payload: UpdateEventPayload): Promise<Event | null> => {
...
// Delete event
export const deleteEvent = async (id: string): Promise<{ success: boolean } | null> => {
  return fetchWithAuth<{ success: boolean }>(`/events/${id}`, { method: "DELETE" });
};
